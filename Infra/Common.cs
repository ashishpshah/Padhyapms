...
namespace PMMS.Infra
{
    public class Common
    {
        ...
        public static string DateTimeFormat_ddMMMyyyyhhmmtt => "dd-MMM-yyyy hh:mm tt";
        ...
        public static async Task<(bool IsSuccess, string Message)> SendEmail(string subject, string recipient_mails, bool isBodyHtml = false, string? body = null, string? templateFile = null, string? templateData = null)
        {
            ...
            try
            {
                if (AppHttpContextAccessor.IsSendMail && recipient_mails != null && recipient_mails.Length > 0)
                {
                    ...
                    {
                        using (StreamReader reader = new StreamReader(System.IO.Path.Combine(AppHttpContextAccessor.WebRootPath, "Email_Templates", templateFile + ".html")))
                        {
                        ...
                        if (!string.IsNullOrEmpty(templateData))
                        {
                            ...
                            foreach (var prop in data.Properties())
                            {
                                ...
                                // Date format: dd-MMM-yyyy (e.g., 06-Mar-2026)
                                if (prop.Name.Equals("fromdate", StringComparison.OrdinalIgnoreCase) ||
                                    prop.Name.Equals("todate", StringComparison.OrdinalIgnoreCase) ||
                                     prop.Name.Equals("leavedate", StringComparison.OrdinalIgnoreCase))
                                {
                                    // If JSON has a real date token or date string, this handles both
                                    if (prop.Value != null && prop.Value.Type == JTokenType.Date)
                                    {
                                        var dt = prop.Value.ToObject<DateTime>();
                                ...
                                if (prop.Name.Equals("fromtime", StringComparison.OrdinalIgnoreCase) ||
                                    prop.Name.Equals("totime", StringComparison.OrdinalIgnoreCase))
                                {
                                    if (prop.Value != null && prop.Value.Type == JTokenType.Date)
                                    {
                                        var dt = prop.Value.ToObject<DateTime>();
                                        value = dt.ToString("HH:mm");   // 24-hour format
                                    }
                                    else if (DateTime.TryParse(value, out var dt))
                                    {
        ...
        //private static List<UserRoleMenuAccess> UserMenuAccess;
        //private static List<UserRoleMenuAccess> UserMenuPermission;

        //public static void Configure_UserMenuAccess(List<UserRoleMenuAccess> userMenuAccess, List<UserRoleMenuAccess> userMenuPermission)
        //{
        ...
        //public static List<UserRoleMenuAccess> GetUserMenuAccesses() => UserMenuAccess;
        //public static List<UserRoleMenuAccess> GetUserMenuPermission() => UserMenuPermission;
